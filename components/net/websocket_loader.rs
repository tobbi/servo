...
use http_loader::{set_cookies_from_headers, set_request_cookies};
...
impl<'a> Handler for Client<'a> {
    ...
    fn on_close(&mut self, code: CloseCode, reason: &str) {
        ...
        let reason = if code == CloseCode::Status {
            "".to_owned()
        } else {
            reason.to_string()
        };
        let _ = self.event_sender.send(WebSocketNetworkEvent::Close(Some(code.into()), reason));
    }
}
        ...
        thread::spawn(move || {
            while let Ok(dom_action) = action_receiver.recv() {
                let _ = match dom_action {
                    ...
                    WebSocketDomAction::Close(None, _) => {
                        sender.close(CloseCode::Status)
                    },
                };
            }
        });
